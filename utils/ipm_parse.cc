...
int main( int argc, char *argv[])
{
  ...
  case TERSE:
    BANNER_SET_NTASKS(banner, job.ntasks);
    BANNER_SET_NHOSTS(banner, job.nodes.size());
    BANNER_SET_NTHREADS(banner, 1);   /* FIXME */
    
    BANNER_SET_CMDLINE(banner, job.cmdline.c_str());
    BANNER_SET_HOSTNAME(banner, job.hostname.c_str());
    BANNER_SET_TSTART(banner, job.start);
    BANNER_SET_TSTOP(banner, job.final);

    BANNER_SET_NREGIONS(banner, 1);  /* FIXME */
    
    job.gstats_procmem(&(banner.procmem));
...
void job_t::gstats_region(banner_t *b, region_t *reg, regstats_t *rstats)
{
  ...
  for( it=taskdata.begin(); it!=taskdata.end(); it++ ) 
    {
      g->dmin = MIN(g->dmin, (*it).regdata[reg].wtime);
      g->dmax = MAX(g->dmin, (*it).regdata[reg].wtime);
      g->dsum += (*it).regdata[reg].wtime;
    }
